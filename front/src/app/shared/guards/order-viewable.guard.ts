...
export class OrderViewableGuard implements CanActivate {
    ...
    subscribe(
      order => {
        this._articlesService.fetchOne(String(order.articleId)).subscribe(
          article => subject.next(article.seller === this._authService.userName || order.buyer === this._authService.userName),
          __ => {
